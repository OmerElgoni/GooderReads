function setTitle(title) {
  ...
  function setCoverImage(coverImageUrl, title) {
    ...
    element.src = coverImageUrl ? coverImageUrl : `https://via.placeholder.com/200x250.?text=${title}jpg`;
}

function setRatings(positiveRatings, negativeRatings) {
    document.getElementById("positiveReviews").textContent = positiveRatings + " positive reviews";
    document.getElementById("negativeReviews").textContent = negativeRatings + " negative reviews";
}
...
async function setDetails() {
    ...
    authorQueryResult = await (
        await authorQueryResult
      ).json();
